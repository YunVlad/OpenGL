...
int process()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        currPos[1].x = 6 + 6*sin(time / 2.0);
        currPos[1].z = 6 + 6*cos(time / 2.0);
        currPos[0].x = 6*sin(time / 2.0);
        ...
        if (preRender)
        {
            ...
            for (unsigned nCurr = 0; nCurr < 3; ++nCurr)
            {
                ...
                for (unsigned i = 0; i < 6; ++i)
                {
                    ...
                    glClearColor(timeSin(time), timeSin(time), timeSin(time), 1.0f);
                    //glClearColor(1.0f, 0.4f + timeSin(time) / 2.0, 0.3f + timeSin(time) / 2.0, 1.0f);
