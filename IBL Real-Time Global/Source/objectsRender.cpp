...
std::vector<unsigned> objectsRender::loadAlbedotexture(std::string path)
{
    std::vector<unsigned> textures;
    textures.push_back(loadTexture(path + "/albedo.jpg"));
    return(textures);
...
void objectsRender::activeAlbedotexture(std::vector<unsigned> textures)
{
    glActiveTexture(GL_TEXTURE1);
    glBindTexture(GL_TEXTURE_2D, textures[0]);;
}
...
{
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_CUBE_MAP, MapsMod[nObj][1]);
    cubeRender();
}
...
void objectsRender::shadowMapGen()
{
...
void objectsRender::shadowMapProcess(glm::vec3 lightPos, shader shadowShader)
{
...
void objectsRender::createMapSetMod(unsigned k)
{
    MapsMod.push_back({ 0, 0 });
    for (unsigned i = 0; i < k; ++i)
        MapsMod.push_back({ 0, 0, 0 });
}

void objectsRender::createEnvMapMod(unsigned nObj)
{
    glBindFramebuffer(GL_FRAMEBUFFER, EnvFBO);
    glEnable(GL_DEPTH_TEST);
    glGenTextures(1, &MapsMod[nObj + 1][0]);
    glBindTexture(GL_TEXTURE_CUBE_MAP, MapsMod[nObj + 1][0]);
...
void objectsRender::saveEnvMapSideMod(unsigned nSide, unsigned nObj)
{
    glViewport(0, 0, mapSizes[0], mapSizes[1]);
    glBindFramebuffer(GL_FRAMEBUFFER, EnvFBO);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_POSITIVE_X + nSide, MapsMod[nObj + 1][0], 0);
}

void objectsRender::createEnvDiffuseMod(shader convShader, glm::vec3 viewPos, unsigned nObj)
{
    glGenTextures(1, &MapsMod[nObj + 1][1]);
    glBindTexture(GL_TEXTURE_CUBE_MAP, MapsMod[nObj + 1][1]);
    for (unsigned int i = 0; i < 6; ++i)
    {
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB16F, (mapSizes[0] / 4), (mapSizes[1] / 4), 0, GL_RGB, GL_FLOAT, nullptr);
    }
    ...
    {
        convShader.mat4Uniform("view", captureViews[i]);
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, MapsMod[nObj + 1][1], 0);
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        cubeRender();
    }
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
...
void objectsRender::createEnvDiffuseMod2(shader convShader)
{
    glGenTextures(1, &MapsMod[0][0]);
    glBindTexture(GL_TEXTURE_CUBE_MAP, MapsMod[0][0]);
    for (unsigned int i = 0; i < 6; ++i)
    {
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB16F, 32, 32, 0, GL_RGB, GL_FLOAT, nullptr);
    }
    ...
    {
        convShader.mat4Uniform("view", captureViews[i]);
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, MapsMod[0][0], 0);
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        cubeRender();
    }
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
...
void objectsRender::createEnvSpecMod(shader prefShader, glm::vec3 viewPos, unsigned nObj)
{
    //glGenTextures(1, &MapsMod[nObj + 1][2]);
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
    glBindTexture(GL_TEXTURE_CUBE_MAP, MapsMod[nObj + 1][2]);
    for (unsigned int i = 0; i < 6; ++i)
    {
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB16F, (mapSizes[0] / 4), (mapSizes[1] / 4), 0, GL_RGB, GL_FLOAT, nullptr);
    }
    ...
    for (unsigned int mip = 0; mip < maxMipLevels; ++mip)
    {
        ...
        {
            prefShader.mat4Uniform("view", captureViews[i]);
            glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, MapsMod[nObj + 1][2], mip);
            glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
            cubeRender();
        }
    }
...
void objectsRender::createEnvBRDFMod(shader brdfShader)
{
    glGenTextures(1, &MapsMod[0][1]);

    glBindTexture(GL_TEXTURE_2D, MapsMod[0][1]);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RG16F, (mapSizes[0] / 2), (mapSizes[1] / 2), 0, GL_RG, GL_FLOAT, 0);
    ...
    glBindRenderbuffer(GL_RENDERBUFFER, captureRBO);
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT24, mapSizes[0] / 2, mapSizes[1] / 2);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, MapsMod[0][1], 0);
...
void objectsRender::createAllEnvMapsMod(shader convShader, shader prefShader, glm::vec3 viewPos, unsigned nObj)
{
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
    createEnvDiffuseMod(convShader, viewPos, nObj);
    createEnvSpecMod(prefShader, viewPos, nObj);
}

void objectsRender::createAllEnvMapsMod2(shader prefShader, glm::vec3 viewPos, unsigned nObj)
{
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
    //createEnvSpecMod(prefShader, viewPos, nObj);
    glGenTextures(1, &MapsMod[nObj + 1][2]);
...
void objectsRender::IBLEnvactiveMod(shader pbrShader, unsigned nObj)
{
    pbrShader.UseShaderProgramm();
    if (useAO)
    {
...
void objectsRender::IBLEnvactiveMod2(shader pbrShader, unsigned nObj)
{
    pbrShader.UseShaderProgramm();
    if (useAO)
    {
