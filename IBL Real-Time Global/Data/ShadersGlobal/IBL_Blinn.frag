...
float BlinnDistribution(vec3 n, vec3 h, float roughness)
{
    float alpha2 = pow(roughness, 4.0);
    float den = 3.14159 * alpha2;
    float num = pow(max(dot(n, h), 0.0), ((2.0/alpha2) - 2));
    return num/den;
}

float BlinnGeometry(float NV, float NL)
{
    return (NV * NL)/max(NV, NL);
}
...
vec3 RenderingEquation(pointLight light, vec3 Norm, vec3 currPos, vec3 viewDir, float roughness, float metallic, vec3 albedo, vec3 F0)
{
    ...
    float NDF = BlinnDistribution(Norm, H, roughness);   
    float G = BlinnGeometry(NdotV, NdotL);   
    vec3 F = Fresnel(H, viewDir, F0);

    vec3 num = NDF * G * F; 
    float den = 4 * max(NdotV, 0.0) * max(NdotL, 0.0) + 0.001;
...
void main()
{
    ...
    vec3 ambient = koefDiffuse * diffuse * 0.05 + specular * 0.5;
    color = ambient + L0;
